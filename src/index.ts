...
events.on(settings.cardsGet, () => {
	...
	storeModel.cardList.forEach(card => {
		...
		ensureElement('.gallery').append(newCard)
	})
})
...
events.on(settings.basketOpened, () => {
	const totalCardsCount = basketModel.getTotalCardsCount()
	basketView.setTotalCardsPrice(totalCardsCount)
	console.log(basketModel)
	
	basketView.setItems(basketModel.cardsList.map((card, index) => {
		const basketItem = new BasketItemView(
			cardBasketTemplateElement,
			events,
			{
				onClick: () => events.emit(settings.basketRemoveCard, card)
			}
		)
...
events.on(settings.basketRemoveCard, (card: ICard) => {
	...
	basketView.setTotalCardsPrice(totalCardsPrice)
	
	basketView.setItems(basketModel.cardsList.map((card: ICard, index: number) => {
		const basketItem = new BasketItemView(
			cardBasketTemplateElement,
			events,
			{
...
events.on(settings.orderSuccessClosed, () => {
	modalView.close()
})
